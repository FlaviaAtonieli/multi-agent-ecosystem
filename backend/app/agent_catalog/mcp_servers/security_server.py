"""Real MCP server exposing the "Segurança da Informação" Agent Skill as a
single tool.

Added after the other three domains as evidence for RFC §5.5 criterio 7 (prova
de extensibilidade plug-and-play). See the note on `DomainLiteral` in
app/agent_manifest/manifest.py for what this addition did and did not touch.
...
Runnable two ways:
  ...
  - as `python -m app.agent_catalog.mcp_servers.security_server`, it speaks
...
from app.agent_catalog.tool_interface import SecuritySkillExecutor, SkillToolCall, SkillToolResult
from app.core.database import SessionLocal
from app.models import AgentSkill, TechnicalRequest, User

mcp = MCPServer(
    "agenthub-security-skill",
    instructions=(
        "Analisa riscos de segurança da informação (exposição de dados, controle de acesso, "
        "superfície de injeção) usando retrieval sobre a base de conhecimento indexada e o "
        "Model Gateway configurado."
    ),
)

_executor = SecuritySkillExecutor()


@mcp.tool()
def executar(entrada: SkillToolCall) -> SkillToolResult:
    """Executa a Agent Skill de Segurança da Informação sobre uma solicitação já qualificada."""
