"""Real MCP server exposing the "Arquitetura de Software" Agent Skill as a single tool.
...
Runnable two ways:
  ...
  - as `python -m app.agent_catalog.mcp_servers.architecture_server`, it speaks
...
from app.agent_catalog.tool_interface import ArchitectureSkillExecutor, SkillToolCall, SkillToolResult
from app.core.database import SessionLocal
from app.models import AgentSkill, TechnicalRequest, User

mcp = MCPServer(
    "agenthub-architecture-skill",
    instructions=(
        "Analisa impactos arquiteturais (acoplamento, limites de módulo, risco estrutural) "
        "usando retrieval sobre a base de conhecimento indexada e o Model Gateway configurado."
    ),
)

_executor = ArchitectureSkillExecutor()


@mcp.tool()
def executar(entrada: SkillToolCall) -> SkillToolResult:
    """Executa a Agent Skill de Arquitetura de Software sobre uma solicitação já qualificada."""
