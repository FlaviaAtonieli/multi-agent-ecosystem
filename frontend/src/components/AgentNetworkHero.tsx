...
export function AgentNetworkHero({ variant = 'login' }: AgentNetworkHeroProps) {
  ...
  useEffect(() => {
    ...
    // Keep non-null aliases for callbacks declared inside this effect.
    // TypeScript does not preserve ref narrowing across nested functions.
    ...
    function drawPointerConnections() {
      ...
      nodes.forEach((node) => {
        ...
        const opacity = (1 - distance / pointerDistance) * 0.38
        safeContext.beginPath()
  ...
  return (
    <section ref={containerRef} className="auth-hero network-hero">
      ...
      <div className="network-status" aria-label="Características do sistema">
        <span><i aria-hidden="true" /> Agent Skills conectadas</span>
        <span><i aria-hidden="true" /> Contratos estruturados</span>
        <span><i aria-hidden="true" /> Rastreabilidade por Trace ID</span>
      </div>

      <small className="network-footnote">
        {variant === 'register'
          ? 'Crie seu acesso para participar do ecossistema.'
          : 'Acesso seguro ao ambiente de agentes especialistas.'}
      </small>
    </section>
  )
}
