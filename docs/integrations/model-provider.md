...
- **Por que 150000**: é uma conta a partir dos tetos já configurados (`LLM_MAX_INPUT_CHARS=12000` ≈ 3000 tokens, `LLM_MAX_OUTPUT_TOKENS=3000` desde 30/08/2026 — subiu de 1200 depois que `openai/gpt-5-mini`, um modelo de raciocínio, esgotava esse teto só com tokens de "pensamento" interno antes de escrever a resposta), não uma calibração de custo real — cada chamada custa no máximo ~6000 tokens, e uma orquestração completa (até ~4 Agent Skills) custa até ~24000 tokens no pior caso, o que dá ~6 execuções completas por usuário por dia. Verificado contra o preço real do único modelo pago hoje na allowlist (`openai/gpt-5-mini`: $0,25/milhão de tokens de entrada, $2,00/milhão de saída — [OpenRouter](https://openrouter.ai/openai/gpt-5-mini)): no pior caso (tudo tratado como saída, mais cara), 150000 tokens/dia custam **~US$0,30/dia por usuário** — com US$10 de crédito, dura mais de 30 dias mesmo com múltiplos usuários testando. Os outros 3 modelos da allowlist são `:free` (custo zero). Essa margem confortável foi confirmada com a autora e o valor foi mantido como está.
